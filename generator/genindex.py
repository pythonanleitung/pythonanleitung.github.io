...
for chapter_index in range(len(chapters_list)):
    ...
    strings['next'] = "[Weiter zu Kapitel {index}: {name}]({filename}) | ".format(index=chapter_index+2, name=next[0], filename=next[1])
...
inhaltsstrings = [" 1. [{0}]({1})".format( x[0], x[1]) for x in chapters_list]
